...
void reply_get (accepted_socket& client_sock, cix_header& header) {
  ...
  if(!load){
    log << "get: "<<header.filename << ": "<< strerror (errno) << endl; 
    header.command = cix_command::NAK; 
    header.nbytes = errno; 
    log << "sending header " << header << endl; 
    send_packet (client_sock, &header, sizeof header); 
  ...
  send_packet (client_sock, buffer, bytes); 
  log << "sent " << bytes << " bytes" << endl; 
}

void reply_put (accepted_socket& client_sock, cix_header& header) {
  ofstream copy;
  copy.open(header.filename);
  char buffer[header.nbytes]; 
  recv_packet (client_sock, buffer, header.nbytes); 
  log << "received payload: " << header.nbytes << " bytes" << endl; 
  if(copy){
    string filename = header.filename; 
    copy.write (buffer, sizeof buffer); 
    copy.close(); 
...
void reply_rm (accepted_socket& client_sock, cix_header& header) {
  ...
  if (rc == 0){
    ...
    string filename = header.filename; 
    log << "rm: " << filename << ": " << strerror (errno) << endl; 
    header.command = cix_command::NAK; 
    header.nbytes = errno; 
    send_packet (client_sock, &header, sizeof header); 
    return; 
